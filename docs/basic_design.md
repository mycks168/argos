...
進捗音声のため別スレッドで応答を読むWeb音声ターンでは、Runner完了だけでは配信済みにせず、SSEの`done`イベントをブラウザへ書き込んだ後に配信済みとする。書き込み前に接続が切れたジョブは未配信回収の対象に残す。切断前までの表示が完成結果の接頭辞なら、回収時に既存メッセージを補完して重複表示を避ける。
...
音声入力の応答では音声チャンクも要求する。対象タイルにフォーカスがない間はブラウザ内に一時保留し、フォーカスされた時点で順番に再生する。ヘッダーのミュートは `POST /api/control` の `mute` / `unmute` を使い、通常・SP・GridとARGOS本体で状態を共有する。SSEでミュート状態を受け取った各画面は再生中と保留中のブラウザ音声を破棄する。ブラウザ再生は通常画面とGridで同じ世代管理付きプレーヤーを使い、16bit PCM WAVをRIFFチャンクから検証・変換する。PCM形式以外はWeb Audio APIの標準デコーダへフォールバックし、どちらでも再生できない場合は画面にエラーを表示する。
...
ダッシュボードでは入力欄横のマイクボタンをタップして録音を開始し、再度タップして停止・送信できる。また、キーボードやUSBペダル等の長押し操作（Push-To-Talk: PTT）に対応し、`Space` キーや `F13`〜`F24` キーを押し続けている間録音を行い、キーを離した時点で録音停止・自動送信する。キー押下直後は物理PTTと同じく画面状態を「録音中」へ変更し、ブラウザのマイク許可待ちや早いキー解放でも録音停止を取りこぼさない。PTT押下時は物理PTT・キーボードともに進行中の読み上げをキャンセルする。ブラウザPTTでは回答ごとの再生世代を無効化し、その回答について既に再生中の音声と後から届く未再生音声を破棄する。一方、SSEは切断せずテキスト差分と完了イベントを最後まで受信するため、画面の回答は欠けない。読み上げ中は画面状態を「読み上げ中」とし、サーバーの待機状態とは別にブラウザの録音・処理・再生状態を優先表示する。操作ボタンの可否も同じブラウザ状態から算出し、表示だけ待機中へ戻る競合を防ぐ。テキスト入力欄（`<input>`、`<textarea>` 等）にカーソル/フォーカスがある場合は通常の文字入力（スペース等）を優先し、PTT動作を抑止する。Safariの `audio/mp4`、Chrome系の `audio/webm`、Opus音声は母艦でffmpegを使ってWAVへ変換し、既存のSTTへ渡す。応答は端末ターンAPIのSSEからWAVチャンクを受信し、Web Audio APIで順次再生する。マイク許可と録音にはHTTPSのセキュアコンテキストを必要とする。
