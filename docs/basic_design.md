...
#### マルチメディア・オーバーレイ（重ね合わせ表示）
外部サービスから `POST /api/events` を経由してオーバーレイ表示イベントを送ることで、ダッシュボード画面に任意の情報を全画面重ね合わせ（半透明モーダル）で表示できる。

1. **オーバーレイ表示イベント (`type: "overlay"`)**
   - `overlay_type`: コンテンツ種別（`map`、`markdown`、`image`、`html` など）。
   - `title`: オーバーレイヘッダーに表示するタイトル文字列。
   - `url`: 表示するコンテンツのURL（例: `/static/map.html`, `/static/reader.html`）。
   - `content` (任意): Markdownなどの生テキストデータ。親ダッシュボードから `postMessage` を経由して iframe の子ページへ安全に転送される。
   - `options` (任意): 緯度経度など、描画時に渡す任意のオプション辞書。

2. **オーバーレイ消去イベント (`type: "clear_overlay"`)**
...
- `map.html` (地図): Leaflet.js を使用して、クエリパラメータで指定された座標をダークモード風の地図へ表示する。現在地は青い円形マーカー、目的地は赤いピンで表示し、各マーカーには常時表示ラベルを付ける。
...
ダッシュボード右上の「閉じる」ボタンをクリックすると、フロントエンド側から自動的に `clear_overlay` イベントが送信され、表示がクリアされる。
...
PTT録音中は本人確認の繰り返し案内と警告音を再生しない。これにより、案内音声がマイクへ回り込んで音声キーワード認識を妨げることを避ける。
