...
音声入力の応答では音声チャンクも要求する。対象タイルにフォーカスがない間はブラウザ内に一時保留し、フォーカスされた時点で順番に再生する。ヘッダーのミュートはGridブラウザ内の再生だけを停止し、保留中の音声も破棄する。ミュート状態はブラウザのローカルストレージへ保存するため、ARGOS本体の物理音声出力設定には影響しない。ブラウザ再生は通常画面とGridで同じ世代管理付きプレーヤーを使い、16bit PCM WAVをRIFFチャンクから検証・変換する。PCM形式以外はWeb Audio APIの標準デコーダへフォールバックし、どちらでも再生できない場合は画面にエラーを表示する。
