...
| `/settings` 画面 | ARGOS 全体（よく変える項目だけ） | すぐ反映 |
...
`/settings` の「音声入力の開始方法」からも切り替えられます。
...
`/settings` の画面から選ぶのが簡単です。設定ファイルで指定する場合はこう書きます。
...
`/settings` では、よく変える項目を再起動なしで変更できます。
...
`/settings` の「現在地の取得元」からも切り替えられます。
...
| 設定を変えても効かない | `config.yaml` の編集は再起動が必要です |
...
`/settings` とダッシュボードのメニューから変えた項目は、再起動なしで反映されます。
